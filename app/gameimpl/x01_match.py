...
class X01Match(MatchManager, MatchVisitTemplate):
    ...
    # This has the potential to be buggy if the match is set first and the players registered after
    def post_init(self):
    ...
    def validate_visit(self, player_index, visit):
        if self.match.last_player_index is player_index:
            return False, "PLayer " + str(player_index + 1) + " is not in the correct sequence. Player ignored."
    ...
    def record_statistics(self, player_index, visit, result):
        ...
        # Calculate first 0 if, and only if, this is the 3rd visit
